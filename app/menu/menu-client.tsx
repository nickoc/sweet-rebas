...
import type { MergedMenuItem } from "@/lib/menu-catalog";
...
function QuickAddCard({ item, image, imagePositionClass, imageWidthClass, onImageClick }: { item: MergedMenuItem; image?: string; imagePositionClass?: string; imageWidthClass?: string; onImageClick?: (src: string, alt: string) => void }) {
...
// Local fallback photos, keyed by the static item id. Used only when the portal
// catalog has no image_url for the item (item.imageUrl takes precedence).
...
export default function MenuClient({ items }: { items: MergedMenuItem[] }) {
  ...
  const grouped = categories.reduce(
    (acc, cat) => {
      acc[cat] = items.filter((item) => item.category === cat.toLowerCase());
      return acc;
    },
    {} as Record<string, MergedMenuItem[]>
  );
